...
class Solution:
    ...
    def longestIncreasingSubsequence(self, nums):
        ...
        n = len(nums)
        len_value = [sys.maxsize] * (n + 1)
        len_value[0] = -sys.maxsize
        
        for i in range(len(nums)):
            index = self.find_position(len_value, nums[i])
            len_value[index] = nums[i]
        
        for i in range(n):
            if len_value[i + 1] == sys.maxsize:
                return i 
        return 0
        
    def find_position(self, arr, num):
        l = 0
        r = len(arr) - 1
        while l + 1 < r:
            m = (l + r) // 2 
...
print(Solution().longestIncreasingSubsequence([-2, -1]))
